/*Creating a Kalplan Meier Estimate Plot

This is an example of a Kaplan Meier survival estimates Plot. In this plot we are comapring the time to death for three different treamtnet regimens. 
Survival estimate is on the Y axis and tiem is represented on the X axis
Each Step in the graph lines represents an event*/
...
*******CREATE;
ods html path="/home/smccawille1/Output" file="KMESurvEst.html"
image_dpi=300 style=htmlblue;

ods graphics on / reset imagename="KMESurvEst" outputfmt=png
attrpriority=color;
...
proc sgplot data=survivalplot;
  ...
  format stratum $trtp.;
  title1 "Kaplan-Meier Estimates for Death";
  run;
