...
%chatgpt(api_key=&api_key.,print=true, out=, parameter= debug, insert_code={data test123; set sashelp.class run;}, insert_error=ERROR: File WORK.RUN.DATA does not exist.);
%chatgpt(api_key=&api_key.,print=true, out=, parameter= efficient, insert_code={data test123; set sashelp.class; run;}, describe_code= That data step works slow.);
